  ...
  render() {
    ...
    return (
      <div className="single-investment-page">
        ...
        <div className="mt-4 col col-md-12 p-4">
          <div className="pt-2 pb-2 d-flex flex-wrap mb-4">
            <div className="overflow-hidden img-section mr-4">
              <img
                src={require(`#/assets/icons/setup-investment.svg`)}
                alt="custom-investment"
                className="img-fluid mr-3 investment-img"
              />
            </div>
            <div className="d-flex flex-column justify-content-center">
              <h3 className="text-blue text-capitalize mb-0">
                {state?.investment.name}
              </h3>
              <p className="text-black text-small text-wrap">
                {state?.investment.description}
              </p>
            </div>
          </div>
          ...
          <div className="row mt-2">
            <div className="col-md-3 mt-4">
              <Card
                classes={`bg-default investment-item cursor-pointer ${
                  investmentList
                    ? investmentList.investments.length
                      ? "multiple"
                      : ""
                    : ""
                }`}
                onclick={this.handleNewInvestment}
              ...
              >
                ...
                <h3
                  className="text-white text-medium pr-4"
                  onClick={this.handleNewInvestment}
                >
                  Create a new {state?.investment.name} plan
                </h3>
              </Card>
            </div>
            {investmentList &&
                ...
                return (
                  ...
                  >
                    <InvestmentItem
                      ...
                      navigateToInvestment={this.handleNavigatetoInvestment}
                    />
                  </div>
                );
              })}
        ...
        {state.investment.type === "collection" ? (
          <Collections />
        ) : state.investment.type === "custom" ? (
          <Custom />
        ) : (
          <Predefined />
        )}
...
const mapStateToProps = (state) => {
  const {
    app: {
      investment: { investments, investmentList },
      profile: {
        userProfile: { data },
      },
    },
  ...
  return {
    loading: getActionLoadingState(
      state,
      actionTypes.GET_INVESTMENT_DETAILS_REQUEST
    ),
    investments,
    investmentList,
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Investment)
);
