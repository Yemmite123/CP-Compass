...
class Custom extends React.Component {
    ...
    handleComfirmation = (e) => {
        ...
        this.state.addEndDate ? this.props.calculateInvestment(info)
            ...
            : (() => { closeOffCanvas(`offcanvas-${state?.investment.id}`); this.toggleConfirmationModal(); })();
    }
    ...
    handleBookWithoutPay = (e) => {
        ...
        const payload = this.state.addEndDate ?
            { title, targetAmount: formatCurrencyToString(target), currency: 'NGN', startDate: moment(startDate).format('YYYY-MM-DD'), endDate: moment(targetDate).format('YYYY-MM-DD'), frequency: frequency.toLowerCase() }
            : { title, targetAmount: formatCurrencyToString(target), currency: 'NGN', startDate: moment(startDate).format('YYYY-MM-DD'), amount: formatCurrencyToString(frequencyAmount), frequency: frequency.toLowerCase() }
    ...
    handleBookWithPay = (autoCharge) => {
        ...
        const payload = this.state.addEndDate ?
            {
                title, targetAmount: formatCurrencyToString(target), currency: 'NGN', startDate: moment(startDate).format('YYYY-MM-DD'), endDate: moment(targetDate).format('YYYY-MM-DD'), frequency: frequency.toLowerCase(), payment,
                initialAmount: formatCurrencyToString(amount), autoCharge
            }
            : {
                title, targetAmount: formatCurrencyToString(target), currency: 'NGN', startDate: moment(startDate).format('YYYY-MM-DD'), frequency: frequency.toLowerCase(), payment,
                ...
                {setupSuccessModal &&
                    <Modal onClose={this.toggleSetupSuccessModal}>
                        ...
                        <div className="px-5">
                            ...
                            <div className="text-center">
                                <div className='mb-3'>
                                    <h5 className="text-success font-bolder">Alright, you're done</h5>
                                </div>
                                <div className="px-3 mt-4">
                            ...
                            <div className="text-center">
                                ...
                                <div className="mt-4">
                                    {fundingSource.map(method => (
                                        <div key={method.value } id={method.value} className={`position-relative d-flex p-3 mb-2 ${selectedMethod === method.value ? "selected" : ""} payment-method`} onClick={this.handleSelectMethod}>
                                            ...
                                            <div id={method.value} className="d-flex mr-3" >
                                                <img id={method.value} src={require(`#/assets/icons/${method.imgUrl}.svg`)} alt="icon" />
                                            </div>
                                            <div id={method.value}>
                                                <h6 id={method.value} className="text-left mb-0 font-bolder">{method.label}</h6>
                                                {method.value === "card" ?
                                                    cards &&
                                                    cards.cards.length > 0 && <p id={method.value} className='text-grey mb-0'>{cards.cards[0].brand} ending in <span id={method.value} className='text-blue'> **** {cards.cards[0].last4}</span></p> :
                                                    <p id={method.value} className='text-grey mb-0'>Available balance <span id={method.value} className='text-blue'> &#x20A6; {walletDetails && walletDetails.wallet.NGN ? walletDetails.wallet.NGN : 0}</span></p>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
