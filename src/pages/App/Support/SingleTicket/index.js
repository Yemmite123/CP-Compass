  ...
  render() {
    ...
    return (
      <div className="single-ticket-page">
        ...
        <div className="message-container border-top rounded-lg border-left border-right">
          {messages && messages.length > 0 && messages.map(message => (
            <MessageItem message={message} key={message.id} viewImage={this.handleShowImage}/>
            ))
          }
        </div>
        <div className=" border rounded-lg message-area p-1">
          <div className="d-flex w-100">
            <form onSubmit={this.submitMessage}>
              <textarea
                placeholder="Send a message..."
                className="border-0 w-100 message-input"
                value={message}
                name="message"
        ...
        {error && typeof error === 'string' && <p className="text-error mt-2 mb-0">{error}</p>}
